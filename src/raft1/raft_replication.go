...
func (rf *Raft) replicate(server int, args AppendEntriesArgs) {
	...
	for !rf.killed() {
		...
		if reply.Term > args.Term {
			...
			DPrintf(tSendAppend, "S%d(%d) -> S%d(%d), append failed at term %d, maybe change to follower", rf.me, rf.currentTerm, server, reply.Term, args.Term)
		...
		DPrintf(tSendAppend, "S%d(%d) -> S%d(%d), append rejected, nextIndex=%d, reply=%+v", rf.me, rf.currentTerm, server, reply.Term, rf.nextIndex[server], reply)
